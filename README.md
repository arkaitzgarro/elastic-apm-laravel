...
|APM_IGNORE_COMMANDS| Comma separated list of commands which run for the lifetime of the process rather than for a single unit of work, and so are not recorded as transactions. The jobs and tasks they run are recorded instead. Defaults to `queue:work,queue:listen,horizon,horizon:work,schedule:work,octane:start`. Add your own long running commands here. |
...
|APM_RENDERSOURCE   | Defaults to `false`. Include source code in query span. |
