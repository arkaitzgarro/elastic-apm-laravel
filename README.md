...
|APM_SPANS_ENABLED  | `true` or `false` defaults to `true`. Set to `false` to stop collecting spans entirely, so only transactions are sent to APM. Overrides the other span settings, including any collector you registered yourself. |
